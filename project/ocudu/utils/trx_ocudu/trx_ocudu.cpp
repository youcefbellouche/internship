...
public:
  ...
  {
    ocudu_assert(buffer.get_nof_samples() == nof_samples, "Unmatched number of samples.");
    ...
    for (unsigned i_channel = 0, nof_channels = buffer.get_nof_channels(); i_channel != nof_channels; ++i_channel) {
      ocuduvec::convert(span<cf_t>(cf_samples[i_channel], nof_samples), buffer.get_reader()[i_channel], scaling_factor);
    }
  }
...
/// Groups parameters that describe a trx_ocudu session.
struct trx_ocudu_session_context {
  /// Parameters.
...
// Keeps persistent trx_ocudu sessions.
static std::vector<std::unique_ptr<trx_ocudu_session_context>> trx_ocudu_sessions;

#if TRX_API_VERSION > 12
static void trx_ocudu_write2(TRXState*         s1,
                             ...
                             TRXWriteMetadata* md)
{
  ...
  if (!affinity) {
    ...
    if (::pthread_setaffinity_np(tself, sizeof(cpu_set_t), &cpuset) != 0) {
      ::perror("pthread_setaffinity_np trx_ocudu_read2");
    }
  ...
  ocudu_assert(s1, "Invalid state.");
  ocudu_assert(md, "Invalid metadata.");

  // Extract context and transmitter interface.
  trx_ocudu_session_context& context = *static_cast<trx_ocudu_session_context*>(s1->opaque);
  ocudu_assert(context.session, "Invalid session.");
...
static int trx_ocudu_read2(TRXState*        s1,
...
static void trx_ocudu_set_tx_gain(TRXState* s1, double gain, int chain)
{
  ocudu_assert(s1, "Invalid state.");
...
static void trx_ocudu_set_rx_gain(TRXState* s1, double gain, int chain)
{
  ocudu_assert(s1, "Invalid state.");
...
static int trx_ocudu_start(TRXState* s1, const TRXDriverParams* p)
{
  ocudu_assert(s1, "Invalid state.");
  ocudu_assert(p, "Invalid parameters.");
  ...
  ocudu_assert(context.rf_port_count > 0, "At least one port is required.");

  // Deduce sampling rate.
  ocudu_assert((p->sample_rate[0].num > 0) && (p->sample_rate[0].den > 0),
  ...
  ocudu_assert(std::all_of(sampling_rates_frac.begin(),
...
static void trx_ocudu_end(TRXState* s1)
{
  ocudu_assert(s1, "Invalid state.");
...
static int
trx_ocudu_get_sample_rate(TRXState* s1, TRXFraction* psample_rate, int* psample_rate_num, int sample_rate_min)
{
  if (psample_rate) {
...
static int trx_ocudu_get_tx_samples_per_packet(TRXState* s1)
{
  ocudu_assert(s1, "Invalid state.");
  trx_ocudu_session_context& context = *static_cast<trx_ocudu_session_context*>(s1->opaque);
...
static int trx_ocudu_get_stats(TRXState* s1, TRXStatistics* m)
{
  ocudu_assert(s1, "Invalid state.");
...
static void trx_ocudu_get_tx_gain(TRXState* s1, double* gain, int channel_num)
{
  ocudu_assert(s1, "Invalid state.");
...
static void trx_ocudu_get_rx_gain(TRXState* s1, double* gain, int channel_num)
{
  ocudu_assert(s1, "Invalid state.");
...
static void trx_ocudu_dump_info(TRXState* s1, trx_printf_cb cb, void* opaque)
{
  ocudu_assert(s1, "Invalid state.");

  cb(opaque, "Using trx_ocudu module\n");
}
...
int trx_driver_init(TRXState* s1)
{
  ...
  ocudu_assert(s1->trx_api_version == TRX_API_VERSION,
  ...
  trx_ocudu_sessions.emplace_back(std::make_unique<trx_ocudu_session_context>());

  // Select context.
  trx_ocudu_session_context& context = *trx_ocudu_sessions.back();
  ...
  if (otw_format_str == "default") {
    ...
    ocudu_terminate("Invalid OTW format {}. Supported formats are: default, sc16.", otw_format_str);
  }
  ...
  if (log_level_char != nullptr) {
    auto value        = ocudulog::str_to_basic_level(std::string(log_level_char));
    context.log_level = value.has_value() ? value.value() : ocudulog::basic_levels::info;
    free(log_level_char);
  } else {
    context.log_level = ocudulog::basic_levels::info;
  }
  ...
  s1->opaque         = &context;
  s1->trx_end_func   = trx_ocudu_end;
  s1->trx_start_func = trx_ocudu_start;
#if TRX_API_VERSION > 12
  s1->trx_write_func2 = trx_ocudu_write2;
  s1->trx_read_func2  = trx_ocudu_read2;
#endif /* TRX_API_VERSION > 12 */
  s1->trx_set_tx_gain_func               = trx_ocudu_set_tx_gain;
  s1->trx_set_rx_gain_func               = trx_ocudu_set_rx_gain;
  s1->trx_get_sample_rate_func           = trx_ocudu_get_sample_rate;
  s1->trx_get_tx_samples_per_packet_func = trx_ocudu_get_tx_samples_per_packet;
  s1->trx_get_stats                      = trx_ocudu_get_stats;
  s1->trx_dump_info                      = trx_ocudu_dump_info;
  s1->trx_get_tx_gain_func               = trx_ocudu_get_tx_gain;
  s1->trx_get_rx_gain_func               = trx_ocudu_get_rx_gain;
