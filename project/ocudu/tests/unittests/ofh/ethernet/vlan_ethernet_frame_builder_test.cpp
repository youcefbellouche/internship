...
TEST(vlan_ethernet_frame_builder_impl_test, build_valid_vlan_ethernet_frame_should_pass)
{
  ...
  // Skip VLAN PCP field.
  params.vlan_config = ether::vlan_parameters{.tci_vid = 2};
...
TEST(vlan_ethernet_frame_builder_impl_test, build_valid_vlan_pcp_ethernet_frame_should_pass)
{
  std::vector<uint8_t> packet = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x80, 0x61, 0x5f, 0x0d, 0xdf,
                                 0xaa, 0x81, 0x00, 0x6f, 0xa0, 0xaa, 0xbb, 0x00, 0x00, 0x00};
  ...
  params.mac_src_address = {0x80, 0x61, 0x5f, 0x0d, 0xdf, 0xaa};
  params.eth_type        = 0xaabb;
  params.vlan_config     = ether::vlan_parameters{.tci_vid = 4000, .tci_pcp = 3};
